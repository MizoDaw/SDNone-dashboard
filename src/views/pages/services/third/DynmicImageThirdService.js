...
import { useAddThirdServiceImage, useDeleteThirdServiceImage, useGetThirdService, useUpdateThirdService, useUpdateThirdServiceImage } from 'api/service/third_service'
...
function DynmicImageThirdService({images=[] , object_Id=1, isFetching , Loading }) {

  const formik = useFormikContext()
  const t  = useTranslation()

  const {mutate:add } = useAddThirdServiceImage()

  const {mutate:update } = useUpdateThirdServiceImage()
  const {mutate , isLoading } = useDeleteThirdServiceImage()
...
export default DynmicImageThirdService
