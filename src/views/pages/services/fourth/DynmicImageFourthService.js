...
import { useAddFourthServiceImage, useDeleteFourthServiceImage, useUpdateFourthService, useUpdateFourthServiceImage } from 'api/service/FourthService'
...
function DynmicImageFourthService({images=[] , object_Id=1 }) {

  const formik = useFormikContext()
  const t  = useTranslation()

  const {mutate:add } = useAddFourthServiceImage()
  const {mutate:update } = useUpdateFourthServiceImage()
  const {mutate , isLoading } = useDeleteFourthServiceImage()
...
export default DynmicImageFourthService; 
