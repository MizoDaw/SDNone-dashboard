...
import DynmicImageThirdService from "./DynmicImageFourthService";
...
const ImageModal = ({ isOpen, setIsOpen, objectToEdit, setObjectToEdit }) => {
  ...
  const {isFetching , data , isLoading:Loading } = useGetFourthService()
      ...
      {objectToEdit && (
        ...
        >
          {(formik) => (
            <Form>
              <ModalBody>
              {isFetching || Loading ?  <SpinnerComponent /> :   <DynmicImageThirdService images={images} object_Id={objectToEdit?.id}/>}              </ModalBody>
              <ModalFooter>
                <Button
                  disabled={isLoading}
