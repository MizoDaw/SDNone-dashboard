...
import { useAddOurWork, useDeleteOurWork, useUpdateOurWork } from 'api/our_works'
...
function AdditionalImagesForm({images=[] , object_Id }) {

  const formik = useFormikContext()
  const t  = useTranslation()

  const {mutate:add } = useAddOurWork()
  const {mutate:update } = useUpdateOurWork()
  const {mutate , isLoading } = useDeleteOurWork()
  ...
  const add_image = (image )=>{
    ...
    formData.append('project_id' ,object_Id)
    add(formData)
  }
